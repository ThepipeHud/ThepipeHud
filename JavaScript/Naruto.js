...
class UserAuth {
    ...
    isFavorite() {
        ...
        return Array.isArray(this.currentUser.favorites) && this.currentUser.favorites.includes('naruto');
    }

    toggleFavorite() {
        if (!this.currentUser) return;
        this.currentUser.favorites = Array.isArray(this.currentUser.favorites) ? this.currentUser.favorites : [];
        const favoriteIndex = this.currentUser.favorites.indexOf('naruto');

        if (favoriteIndex === -1) {
            this.currentUser.favorites.push('naruto');
        } else {
