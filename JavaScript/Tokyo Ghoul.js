...
class UserAuth {
    ...
    isFavorite() {
        ...
        return Array.isArray(this.currentUser.favorites) && this.currentUser.favorites.includes('tokyo-ghoul');
    }

    toggleFavorite() {
        if (!this.currentUser) return;
        this.currentUser.favorites = Array.isArray(this.currentUser.favorites) ? this.currentUser.favorites : [];
        const favoriteIndex = this.currentUser.favorites.indexOf('tokyo-ghoul');

        if (favoriteIndex === -1) {
            this.currentUser.favorites.push('tokyo-ghoul');
        } else {
